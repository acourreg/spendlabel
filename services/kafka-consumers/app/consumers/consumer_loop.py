...
def run_consumer(paradigm: str, classifier, idle_timeout: float = 15.0,
    ...
    buf: list[tuple] = []  # (ocid, title, description, value_gbp, supplier_name)
    processed = 0
    ...
    def flush() -> None:
        ...
        preds = classifier.classify([(b[1], b[2], b[3]) for b in buf])
        batch_ms = (time.perf_counter() - t0) * 1000.0
        # Per-paradigm latency (audit fix B6): compute-bound paradigms (ML heads,
        # keyword, solver) report amortized wall/size for their real batch. IO-bound
        # paradigms (LLM/webhook) run their batch CONCURRENTLY, so wall/size would
        # understate true cost — they expose pop_latencies_ms() with the real
        # per-request times instead.
        pop = getattr(classifier, "pop_latencies_ms", None)
        per_item = pop() if callable(pop) else None
        if not per_item or len(per_item) != len(buf):
            per_item = [batch_ms / len(buf)] * len(buf)
        rows = [
            (b[0], paradigm, preds[i], None, None, per_item[i], b[3], b[4])
    ...
    try:
        while True:
            ...
            if not ocid:
                continue
            buf.append((ocid, rec.get("title"), rec.get("description"),
                        rec.get("value_gbp"), rec.get("supplier_name")))
