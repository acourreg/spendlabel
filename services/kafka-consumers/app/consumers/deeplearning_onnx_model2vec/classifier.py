"""deeplearning_onnx_model2vec classifier — classification only.

Deep-learning head: Model2Vec static embeddings (no transformer) feed a trained MLP
(``mlp_m2v.onnx``). Same MLP recipe as the HuggingFace variant, but on Model2Vec
vectors — ~100x cheaper embedding. No Kafka, no DB (the shared
...
PARADIGM = "deeplearning_onnx_model2vec"
HEAD_FILE = "mlp_m2v.onnx"


class Classifier:
    """Model2Vec embeddings -> MLP -> 2-digit CPV code."""
